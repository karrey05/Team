...
files_cf = glob.glob('/media/Disk3T/Team/dwu/viirs/data/subset/cf/*asc')
...
#light_t={'Beijing':[],'Shanghai':[],'Xian':[],'Guangzhou':[],'Nanjing':[],'Tangshan':[],'Shijiazhuang':[],'Zibo':[],'Zhengzhou':[]}
time_t=[]
...
for f in files:
	...
	for c in range(len(city)):
		...
		light_t[city[c]].append(np.mean(data[id_box]))
		ct_t[city[c]].append(len(data[id_box]))
		print(len(data[id_box]))

#fdata={'data':data,'lons':lons,'lats':lats,'light_t':light_t,'ct_t':ct_t}
fdata={'cities':city,'time_t':time_t,'light_t':light_t,'ct_t':ct_t}

np.save('city_lights_'+str(dbox)+'.npy',fdata)
